...
try {
    ...
    $("#weather").innerHTML = `<span class="weather-icon">☀</span><div><strong>Parana race weekend</strong><p>${Math.round(weather.main.temp)}°C · ${weather.weather[0].description}</p></div>`;
} catch (error) {
    $("#weather").textContent = "Weather data is temporarily unavailable.";
}

try {
    const news = await get("data/news.json");
    $("#news").innerHTML = news.slice(0, 3).map((item) => `<article class="card"><h3>${item.title}</h3><p>${item.date}</p><a href="${item.url}" target="_blank">Read at ACTC</a></article>`).join("");
} catch (error) {
    $("#news").textContent = "News unavailable.";
}

try {
    const drivers = await get("data/drivers.json");
    $("#featured-drivers").innerHTML = drivers.slice(0, 3).map((driver) => `<article class="card driver-mini"><img src="images/${driver.image}" alt="${driver.name}" loading="lazy"><div><img class="brand-logo" src="images/${driver.brandLogo}" alt="${driver.brand} logo"><h3>#${driver.number} ${driver.name}</h3><p>${driver.position}º · ${driver.points} pts</p></div></article>`).join("");
} catch (error) {
    $("#featured-drivers").textContent = "Driver data unavailable.";
}
